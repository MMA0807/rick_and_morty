...
import 'package:rick_and_morty/core/util/gql_query.dart';
import 'package:rick_and_morty/data/models/models.dart';
import 'package:rick_and_morty/core/error/error.dart' as excptn;

abstract class IHomeRemoteDataSource {
  Future<List<CharacterModel>> getCharacters(int page);

  Future<List<CharacterModel>> getCharacter(List<int> ids);
...
class HomeRemoteDataSource implements IHomeRemoteDataSource {
  ...
  @override
  Future<List<CharacterModel>> getCharacter(List<int> ids) async {
    try {
      final result = await _client.query(
        QueryOptions(
          document: gql(GqlQuery.characterByIdsQuery),
          variables: {"ids": ids},
        ),
      );
